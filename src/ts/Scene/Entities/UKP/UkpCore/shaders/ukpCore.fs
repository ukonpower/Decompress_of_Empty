...
vec2 D( vec3 p ) {
	...
	vec2 d = vec2( sdSphere( p, 0.3 + sin( t * 0.2 ) * 0.1 ), 0.0 );
	...
	for( float i = 0.0; i < 3.0; i ++ ) {
		...
		vec3 offset = vec3( 
			sin( t * 2.0 * sin( i ) ),
			sin( t * sin( i ) + i ),
			sin( -t * 0.8 + i * PI ) ) * ( i < 3.0 ? 0.6 : 1.4 + sin( uTime + i * 2.3 ) * 0.2
		);
		vec3 cp = p + offset * 0.3;

		d.x = smoothAdd( d.x, sdSphere( cp, 0.2 + sin( t * 0.2 + i ) * 0.2 ), 0.8 );
...
void main( void ) {
	...
	if( dist.y == 0.0 ) {
		...
		float dnv = dot( normal, -rayDir );
 		...
 		outEmission += vec3( 1.0, 1.0, 1.0 ) * ( 1.0 - dnv);
